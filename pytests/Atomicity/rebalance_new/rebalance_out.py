...
class RebalanceOutTests(RebalanceBaseTest):
    ...
    def rebalance_out_after_ops(self):
        ...
        if (self.doc_ops is not None):
            ...
            for task in tasks:
                self.task.jython_task_manager.get_task_result(task)
        #=======================================================================
        # self.sleep(20,"Sleeping for 20 seconds")
        #=======================================================================
        servs_out = [self.cluster.servers[self.num_servers - i - 1] for i in range(self.nodes_out)]
        #=======================================================================
        ...
        #=======================================================================
        self.sleep(120,"Starting new_failover_stats")
    ...
    def rebalance_out_with_failover(self):
        ...
        # self.bucket_util._wait_for_stats_all_buckets()
        #=======================================================================
        self.sleep(180, "Sleep for 180 seconds")
    ...
    def rebalance_out_get_random_key(self):
        ...
        # self.bucket_util.verify_unacked_bytes_all_buckets()
        #=======================================================================
        self.sleep(60, "Wait for cluster after rebalance")
    ...
    def incremental_rebalance_out_with_ops(self):
        ...
        for i in reversed(range(1, self.num_servers, 2)):
            tasks=[]
            tasks.append(self.task.async_rebalance(self.cluster.servers[:i], [], self.cluster.servers[i:i + 2]))
            if (self.doc_ops is not None):
                self.log.info("Passed this")
                # gen_create = self.get_doc_generator(self.num_items + 1, self.num_items * 3 / 2)
                if ("update" in self.doc_ops):
                    tasks.append(self.task.async_load_gen_docs_atomicity(
                        self.cluster,self.bucket_util.buckets, self.gen_update, "rebalance_update",0,
            ...
            self.sleep(120,"Sleep for 120 seconds")
            #===================================================================
            # self.bucket_util.verify_cluster_stats(self.num_items)
            #===================================================================
        #=======================================================================
    ...
    def rebalance_out_with_queries(self):
        ...
        tasks = []
        for bucket in self.bucket_util.buckets:
            temp = self.bucket_util.make_default_views(self.default_view, num_views, is_dev_ddoc, 
        ...
        active_tasks = self.cluster_util.async_monitor_active_task(self.cluster.servers, "indexer", "_design/" + prefix + ddoc_name,
                                                              wait_task=False)
        ...
        for bucket in self.bucket_util.buckets:
            self.bucket_util.perform_verify_queries(num_views, prefix, ddoc_name,self.default_view_name,query, bucket=bucket, wait_time=timeout,
