...
from Atomicity.failover.failoverbasetest import FailoverBaseTest
...
#                                               check_ep_items_remaining=True)
    ...
    def load_all_buckets(self, gen, op):
        task = self.task.async_load_gen_docs_atomicity(self.cluster, self.bucket_util.buckets,
                                             ...
                                             retries=self.sdk_retries, transaction_timeout=self.transaction_timeout, commit=self.transaction_commit, durability=self.durability)
    ...
    def run_mutation_operations(self):
        ...
        if("delete" in self.doc_ops):
            self.load_all_buckets(self.gen_delete, "rebalance_delete")
    ...
    def run_mutation_operations_after_failover(self):
        ...
        if ("delete" in self.doc_ops):
            self.load_all_buckets(self.afterfailover_gen_delete, "rebalance_delete")
